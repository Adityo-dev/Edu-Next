...
(async () => {
  ...
  try {
    ...
    const lintFixResult = runCommand(`npm run lint:fix -- ${filesToCheck.join(' ')}`);
    spinner.success(chalk.green(emoji('⚙️') + ' Lint fixes applied!'));

    // Run format check
    spinner.start('Running format check...');
    const formatResult = runCommand(`npm run format -- ${filesToCheck.join(' ')}`);
    spinner.success(chalk.green(emoji('✅') + ' Format checks passed!'));

    // Run format:fix
    spinner.start('Applying format fixes...');
    const formatFixResult = runCommand(`npm run format:fix -- ${filesToCheck.join(' ')}`);
